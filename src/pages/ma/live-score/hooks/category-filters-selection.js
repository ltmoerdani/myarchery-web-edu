...
function useCategoryFiltersSelection(eventId) {
  ...
  const { activeCategory, activeTeam, activeCategoryDetail, activeSelectType } = _getActiveData(filters);
  ...
  const scoreType = (score_type) => {
    dispatch({ type: "SELECT_SCORE_TYPE", payload: score_type });
  };
...
function filterReducer(state, action) {
  ...
  if (action.type === "SELECT_SCORE_TYPE") {
    return {
      ...state,
      scoreTypes: action.payload,
    };
  }
...
function _makeInitialState(state, payload) {
  ...
  const defaultScoreType = state.scoreTypes

  return {
    category: defaultCategory,
    teams: defaultTeam,
    categoryDetails: groupedCategories,
    scoreTypes: defaultScoreType,
  };
}
